...
function Pipeline:setOption(name, value)
	if(Pipeline.DefaultSettings[name] ~= nil) then
		self[name] = value;
		-- Refresh parser/unparser if LuaVersion changed
...
function Pipeline:apply(code, filename)
	...
	if self.__azure_vm_result then
		code = self.__azure_vm_result
		self.__azure_vm_result = nil
	else
		-- Rename Variables Step
		self:renameVariables(ast);
		code = self:unparse(ast);
		code = "-- AzureXPino | v1.1 | Protected\n" .. code
	end
